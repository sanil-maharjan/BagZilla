...
export default function ShopProductCard({ product }) {
    const { addToCart, isWishlisted, toggleWishlist, offers } = useProducts();
    const offer = offers.find(o => o.product_id === product.id || o.productId === product.id);
    return (
        <motion.div
            layout
        ...
        >
            <Link href={`/product/${product.id}`} className="relative h-64 overflow-hidden group block">
                <img 
                    src={product.image} 
                    alt={product.name} 
            ...
            <div className="p-6 flex flex-col flex-grow">
                ...
                <div className="flex items-center justify-between mt-auto">
                    ...
                    <div className="flex gap-2">
                        <Link href={`/product/${product.id}`} className="p-2 text-[var(--brand-primary)] border border-[var(--brand-primary)]/30 rounded-full hover:bg-[var(--brand-primary)] hover:text-white transition-colors">
                            <Eye size={18} />
                        </Link>

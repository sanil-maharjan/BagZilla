...
export function useCheckout({ product, qty }) {
    ...
    const productOffer = product ? offersData.find(o => o.product.id === product.id) : null;
    ...
    const handleApplyPromo = () => {
        ...
        // Find promo code
        const offer = offersData.find(o => o.code.toUpperCase() === promoCode.toUpperCase());
        ...
        if (offer.product.id !== product.id) {
            setPromoError(`This promo code is only valid for ${offer.product.name}`);
            setAppliedPromo(null);
            return;
        }

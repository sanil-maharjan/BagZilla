    ...
    return (
        ...
        >
            ...
            <div className="relative z-10">
                ...
                <Link to={`/product/${offer.product.id}`} className="flex items-center gap-4 bg-gray-50 hover:bg-gray-100 transition-colors p-4 rounded-2xl border border-gray-100 group/item cursor-pointer block">
                    <img
                    ...
                    />
                    <div>
                        <p className="font-bold text-gray-800 line-clamp-1 group-hover/item:text-[var(--brand-primary)] transition-colors">{offer.product.name}</p>
                        <p className="text-sm text-gray-500 mt-1">
                            Starting from <span className="font-semibold text-[var(--brand-primary)]">रू {offer.product.price}</span>
                        </p>
                    </div>
                </Link>

                <Link to={`/product/${offer.product.id}`} className="mt-8 w-full bg-[var(--brand-primary)] hover:bg-[var(--brand-secondary)] text-white font-bold py-4 px-6 rounded-xl shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2 group/btn">
                    Shop Now 
